		...
		<div class="col-sm-9">
			...
			<div class="row panel-body">
				...
				@foreach(array_chunk($celebrities->all(), 6) as $videos)
					...
					<div class="cardheader banner banner_{{$val->id}}" style="background: url('{{$val->banner}}');">
										<script>
											if('{{$val->banner}}'.length > 0) {
	  {{--                        $('.banner_{{$val->id}}').css('background', 'url({{$val->banner}})');--}}
											} else {
												$('.banner_{{$val->id}}').css('background', 'url({{$reciter_default_banner}})');
									...
									<div class="info">
										<div class="title">
											<a class="colorWhite font18" href="/reciter/{{str_replace(' ', '-', $val->name)}}">{{$val->name}}</a>
										</div>
