...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
          <form method="POST" action="/admin/combination-images/{{$data->id}}" accept-charset="UTF-8" enctype="multipart/form-data" class="form-horizontal form-label-left">
            {{csrf_field()}}
                ...
                <select name="city_id" id="" class="form-control city">
                    @foreach($cities as $key => $val)
                      @if($val->id == $data->city_id)
                        <option value="{{$val->id}}"  data-places="{{json_encode($val->places)}}" selected>{{$val->name}}</option>
                      @else
                        <option value="{{$val->id}}"  data-places="{{json_encode($val->places)}}">{{$val->name}}</option>
                      @endif
                    @endforeach
                </select>
              </div>
            ...
            <div class="form-group">
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                <input type="checkbox" name="parent" id="parentImage" @if($data->is_parent) checked @endif>
              </div>
            </div>


            <div class="form-group" @if($data->is_parent) style="display: none" @endif id="placeNameThumbnail">
              <label class="control-label col-md-3 col-sm-3 col-xs-12" for="category_title">
                Place Name
                <span class="required">*</span>
              </label>
...
@section('scripts')
  <script>
		$('.city').on('change', function () {
			...
			var html = "<select>"
			html += "<option > Select Place </option>";
        ...
        $(document).on('click', '#parentImage', function () {
            if($(this).is(':checked')) {
